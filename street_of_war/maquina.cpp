...
void maquina::keyPressEvent(QKeyEvent *puss)
{
    if(pausa){
...
void maquina::keyReleaseEvent(QKeyEvent *push)
{
...
void maquina::setnivel(int x)
{
    nivel=x;
}

void maquina::setvida1(int x)
{
    vida1=x;
}

void maquina::setvida2(int x)
{
    vida2=x;
}
...
void maquina::nivel3(int a)//1:izquierda 2:saltar 3:derecha 4:agacharse 5:patada 6:golpe
{
...
void maquina::nivel4(int a)//1:izquierda 2:saltar 3:derecha 4:agacharse 5:patada 6:golpe
{
    if(!jug1->collidesWithItem(jug2))jug2->setVelx(0);
    switch (a) {
    case 1:if((rand()%4)==1){
...
void maquina::on_progressBar_valueChanged(int value)
{
    ui->progressBar->setValue(value);
}

void maquina::on_progressBar_2_valueChanged(int value)
{
...
void maquina::setNombre(const string &value)
{
    nombre = value;
}

void maquina::opcion4()
{
    ui->button->hide();
    timer->start(500);
    time2->start(100);
...
void maquina::anim()
{
    ...
    if(vida1<=0){
        pausa=0;
        jug1->setPixmap(pictureB[0][5]);//QPixmap(":/caido1.0.png"));
...
void maquina::opcion2(){
    principal *N=new principal;
    N->show();
    close();
}
void maquina::opcion3(){
    nivel++;
    auxpelea=0;
    nivel3a=0;
    vida1=100;
...
void maquina::on_actionGuardar_triggered()
{
    ofstream txt4;
    txt4.open(nombre.c_str(),ios::out);
    txt4<<"";
    txt4.close();
    ofstream txt5;
    txt5.open(nombre.c_str(),ios::app);
    txt5<<vida1<<endl;
    txt5<<vida2<<endl;
    txt5<<nivel<<endl;
    txt5<<jug1->getPosx()<<endl;
    txt5<<jug2->getPosx();
    txt5.close();
}
//******************************************** PAUSA *********************************************************
void maquina::on_actionPausar_triggered()
{
    timer->stop();
    time2->stop();
    pausa=0;
    QString a="    Run    ";
    ui->button->setText(a);
    ui->button->setStyleSheet("background-color: blue");
    ui->button->setGeometry(450,200,300,80);
    ui->button->show();
    connect(ui->button,SIGNAL(clicked(bool)),this,SLOT(opcion4()));
}
